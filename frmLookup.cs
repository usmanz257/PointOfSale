...
namespace PointOfSale
{
    public partial class frmLookup : Form
    {
        ...
        public void LoadProducts()
        {
            ...
            cn.Open();
            cm = new SqlCommand("Select p.pcode,p.barcode, p.pdesc, b.brand, c.category,p.cost, p.price, p.qty, p.disc_per from tblProduct as p inner join tblBrand as b on b.id = p.bid inner join tblCategory as c on c.id = p.cid where p.pdesc like '" + txtSearchProduct.Text + "%'", cn);
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
                i++;
                dataGridProduct.Rows.Add(i, dr[0].ToString(), dr[1].ToString(), dr[2].ToString(), dr[3].ToString(), dr[4].ToString(), dr[5].ToString(), dr[6].ToString(), dr["disc_per"].ToString(), dr[7].ToString());
            }
            cn.Close();
        }
